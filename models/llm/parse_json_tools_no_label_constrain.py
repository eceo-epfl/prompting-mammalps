...
### Basic functions
def load_video_detections(json_file: Path | str) -> VideoDict:
    """
...
def load_video_info(json_file: Path | str) -> InfoDict:
    with open(json_file, "r") as f:
        content = json.load(f)
...
def get_tracks_from_json(json_file: str) -> List[IndividualTrack]:
    ...
    Returns:
        List[IndividualTrack]: List of individual tracks present in the JSON file.
    """
...
@tool
def load_json_from_id(file_id: str) -> dict:
    """
    Load the content of a json file given its id
    It contains two keys: 
    "info" -> high level information about the file.
    "frames" a list of dictionnaries summarizing frame-level detections:
        ...
        [
            {
                "frame_id": 1,
                "detections": [
                    {"track_id": 1, "segment_id": 1, "attributes": ["Action": ..., "Action2": ..., "DAge": ..., "DSex": ..., "Species": ...]},
                    {"track_id": 2, "segment_id": 1, "attributes": ["Action": ..., "Action2": ..., "DAge": ..., "DSex": ..., "Species": ...]},
                    ...
                ]
            },
            ...
    ...
    Args:
        file_id (str): id corresponding to the input json file

    Returns:
        dict: the content of the json file as a python dictionnary
...
@tool
def get_tracks_from_file_id(file_id: str) -> List[IndividualTrack]:
    """
...
@tool
def get_weather_condition_from_file_id(file_id: str) -> str:
    """
    Retrieve the weather condition from a given JSON file id.
    Args:
        file_id (str): id corresponding to input json file

    Returns:
        str: The weather condition of the file_id.
    """
    json_file_path = str(next(JSON_FOLDER.rglob(f"*{file_id}.json")))
...
def check_contains_weather_condition(
    file_id: str, weather_condition: str
) -> bool:
    """
    Check if the file_id contains the specified weather condition
    Args:
        file_id (str): id corresponding to input json file
        weather_condition (str): The weather condition to check.
    Returns:
        bool: True if the video contains the specified weather condition, False otherwise.
    """
    video_weather = get_weather_condition_from_file_id(file_id)
...
def get_tracks_from_attribute(
    ...
    Args:
        individual_tracks (List[IndividualTrack]): List of individual tracks.
...
def check_enum_type(value, enumType, allow_none: bool = False):
    ...
    if not isinstance(value, enumType):
        available = [e.value for e in enumType]
        msg = (
            f"invalid enum member {value!r}: expected an instance of {enumType.__name__}. "
...
def get_tracks_from_species(
    ...
    Args:
        ...
        species_name (str): the species name to retrieve tracks from.
...
def get_tracks_from_action(
    ...
    Args:
        individual_tracks (List[IndividualTrack]): a list of individual tracks
        action_name (str): the action name to retrieve tracks from.
...
def get_tracks_from_activity(
    ...
    Args:
        individual_tracks (List[IndividualTrack]): a list of individual tracks
        activity_name (str): the activity name to retrieve tracks from.
...
def get_deer_tracks_from_age(
    ...
    Args:
        ...
        deer_species (Optional[str]): the deer species name to retrieve tracks from. If None, applies to both red deer and roe deer.
...
@tool
def check_tracks_contain_species(
    individual_tracks: List[IndividualTrack], species_name: str
) -> bool:
    """
...
@tool
def check_tracks_contain_action(
    individual_tracks: List[IndividualTrack], action_name: str
) -> bool:
    """
...
@tool
def check_tracks_contain_activity(
    individual_tracks: List[IndividualTrack],
    activity_name: str,
) -> bool:
    """
...
@tool
def check_tracks_contain_deer_age(
    individual_tracks: List[IndividualTrack],
    age: str,
    deer_species: Optional[str] = None,
) -> bool:
...
@tool
def check_tracks_contain_adult_deer_sex(
    individual_tracks: List[IndividualTrack],
    sex: str,
    deer_species: Optional[str] = None,
) -> bool:
...
def get_unique_actions_from_tracks(
    ...
    Returns:
        set[str]: the unique actions present in the individual tracks
    """
    actions = []
    for track in individual_tracks:
        for segment in track:
            if "attributes" in segment[0]:
                if "Action" in segment[0]["attributes"]:
                    actions.append(str(segment[0]["attributes"]["Action"]))
                if (
                    "Action2" in segment[0]["attributes"]
                    and segment[0]["attributes"]["Action2"] != "none"
                ):
                    actions.append(str(segment[0]["attributes"]["Action2"]))
...
def get_unique_activities_from_tracks(
    ...
    Returns:
        set[str]: the unique activities present in the individual tracks
    """

    return set(
        str(segment[0]["attributes"]["Activity"])
        for track in individual_tracks
        for segment in track
...
def check_track_contains_continuous_sequence(
    ...
    Args:
        ...
        attributes_sequence (List[str]): an ordered list representing a sequence of actions or activities
...
def get_action_sequences_from_tracks(
    ...
    Returns:
        List[List[str]]: The action sequences for each individual track
    """
            ...
            if "attributes" in segment[0]:
                if "Action" in segment[0]["attributes"]:
                    if (
                        ...
                        track_actions.append(str(segment[0]["attributes"]["Action"]))
                if (
                    "Action2" in segment[0]["attributes"]
                    and segment[0]["attributes"]["Action2"] != "none"
                ):
                    if (
                        ...
                        track_actions.append(
                            str(segment[0]["attributes"]["Action2"])
                        )
